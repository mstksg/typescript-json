...
reAssignPrim :: (r -> a) -> TSPrim a -> TSType 'Z k b -> Maybe (r -> Either Text b)
reAssignPrim z = \case
    TSBoolean -> \case
...
reAssign :: TSType 'Z k a -> TSType 'Z j b -> Maybe (a -> Either Text b)
...
loopReAssign
    :: forall a b k l. ()
    => TSType 'Z l a
    -> (forall j c. TSType 'Z j c -> Maybe (a -> Either Text c))
    -> TSType 'Z k b
    -> Maybe (a -> Either Text b)
loopReAssign z f = go
  where
    go :: TSType 'Z j c -> Maybe (a -> Either Text c)
    go = \case
...
reAssignIsObj :: TSType 'Z 'IsObj a -> TSType 'Z k b -> Maybe (a -> Either Text b)
...
assembleKeyVal mp (PreT p) = go p
  where
    go :: Ap (Pre d (ObjMember (TSType_ 'Z))) c -> Maybe (a -> Either Text c)
    go = \case
