...
data TSType :: Type -> Type -> Type where
    TSArray  :: TSType n a -> TSType n (Vector a)
    TSTuple  :: NP (TSType n) as -> TSType n (NP I as)
    TSObject :: NP (K Text :*: TSType n) as -> TSType n (ObjFields as)
    TSUnion  :: NP (TSType n) as -> TSType n (NS I as)
    TSNamed  :: n -> TSType n a -> TSType n a
    -- hmm...
    TSIntersection :: NP (ObjType n) as -> TSType n (ObjFields (Concat as))
    TSPrimType :: TSPrim a -> TSType n a
...
ppType :: TSType Void a -> PP.Doc x
ppType = \case
    TSArray t   -> ppType t PP.<+> "[]"
    TSTuple ts  -> PP.encloseSep "[" "]" ", " (npToList ppType ts)
    TSObject ts -> PP.encloseSep "{" "}" "," $
      npToList (\(K n :*: x) -> PP.pretty n PP.<+> ": " PP.<+> ppType x) ts
    TSUnion ts  -> PP.encloseSep "" "" " | " (npToList ppType ts)
    TSNamed v _ -> absurd v
    TSIntersection ts -> PP.encloseSep "" "" " & " (npToList (ppType . getObjType) ts)
    TSPrimType p -> ppPrim p
...
typeToValue :: TSType n a -> a -> A.Value
typeToValue = \case
    TSArray t   -> A.Array . fmap (typeToValue t)
    TSTuple ts  -> A.Array . V.fromList . npToList2 (\t (I x) -> typeToValue t x) ts
    TSObject ts -> A.object . npToList2 (\(K k :*: t) (I x) -> k A..= typeToValue t x) ts . getObjFields
